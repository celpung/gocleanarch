...
func (h *CompanyHandler) CreateCompany(w http.ResponseWriter, r *http.Request) {
	...
	httpx.WriteJSON(w, http.StatusCreated, companyToResponse(*company))
}
...
func (h *CompanyHandler) GetCompany(w http.ResponseWriter, r *http.Request) {
	...
	httpx.WriteJSON(w, http.StatusOK, companyToResponse(*company))
}
...
func (h *CompanyHandler) ListCompanies(w http.ResponseWriter, r *http.Request) {
	...
	resp := dto.ListCompaniesResponse{
		Data: companiesToResponses(companies),
		Meta: dto.PagingMeta{
			Page:       page,
			Limit:      limit,
			TotalItems: total,
...
func respondCompanyError(w http.ResponseWriter, err error) {
	...
	case errors.Is(err, porterrors.ErrCompanyNameExists):
...
func companyToResponse(company entity.Company) dto.CompanyResponse {
...
func companiesToResponses(companies []entity.Company) []dto.CompanyResponse {
	result := make([]dto.CompanyResponse, 0, len(companies))
	for _, c := range companies {
		result = append(result, companyToResponse(c))
	}
	return result
}
